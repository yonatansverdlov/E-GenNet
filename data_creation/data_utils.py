...
def create_hard() -> Data:
    """
    Creates a dataset with two complete graphs and their associated positions, adjacency matrices, 
    and features.

    Returns:
        Data: The two data points with complete graph structure.
    """
    def create_complete_graph():
        """Creates a complete graph adjacency matrix for k nodes."""
        adjacency_matrix = torch.ones((6, 6), dtype=torch.long) - torch.eye(6, dtype=torch.long)
        return adjacency_matrix


    # Graph 0
    edge_index_0 = create_complete_graph().squeeze(0).unsqueeze(-1).unsqueeze(-1)
    pos1 = torch.tensor([
        [ 6.73555740e-17,  1.10000000e+00,  2.20000000e+00],
    ...
    points = Data(
        pos=torch.stack([pos1, pos2]),
        adjacency_matrix=torch.stack([edge_index_0, edge_index_0]),
        label=torch.tensor([0, 1], dtype=torch.long).view(2,),
        node_features=torch.zeros((2, 6, 1), dtype=torch.int),
        edge_attr=torch.zeros((2, edge_index_0.size(1), 1, 1), dtype=torch.int)
    )
