...
def _run_experiment(model, train_loader, val_loader, test_loader, n_epochs=200, verbose=True, device='cpu'):
    total_param = 0
    for param in model.parameters():
        total_param += np.prod(list(param.data.size()))
    model = model.to(device)

    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001) # Was 0.0001
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau( # patience=25
